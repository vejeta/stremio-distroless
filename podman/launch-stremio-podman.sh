#!/bin/bash
# Stremio Launcher for Podman
# Podman provides rootless containers with enhanced security
# GPLv3 License - https://github.com/vejeta/docker-stremio-wolfi

set -euo pipefail

# ============================================================================
# Configuration
# ============================================================================

USER_UID="${UID:-$(id -u)}"
USER_GID="${GID:-$(id -g)}"
IMAGE_NAME="${STREMIO_IMAGE:-ghcr.io/vejeta/stremio-wolfi:latest}"

# X11 authentication
XAUTH_TMP="${TMPDIR:-/tmp}/.podman-stremio.xauth.$$"
touch "$XAUTH_TMP"
chmod 600 "$XAUTH_TMP"
xauth nlist "$DISPLAY" | sed -e 's/^..../ffff/' | xauth -f "$XAUTH_TMP" nmerge - 2>/dev/null || true

trap 'rm -f "$XAUTH_TMP"' EXIT INT TERM

# Device groups
VIDEO_GROUPS=""
if [ -e /dev/dri/card0 ]; then
    VIDEO_GID=$(stat -c '%g' /dev/dri/card0)
    VIDEO_GROUPS="${VIDEO_GROUPS}--group-add=${VIDEO_GID} "
fi

if [ -e /dev/dri/renderD128 ]; then
    RENDER_GID=$(stat -c '%g' /dev/dri/renderD128)
    VIDEO_GROUPS="${VIDEO_GROUPS}--group-add=${RENDER_GID} "
fi

AUDIO_GROUPS=""
if [ -e "/run/user/${USER_UID}/pulse/native" ]; then
    PULSE_GID=$(stat -c '%g' "/run/user/${USER_UID}/pulse/native")
    AUDIO_GROUPS="--group-add=${PULSE_GID}"
fi

echo "🔒 Launching Stremio with Podman (rootless)..."
echo "   User: ${USER_UID}:${USER_GID}"
echo "   Display: ${DISPLAY}"
echo "   Image: ${IMAGE_NAME}"

# ============================================================================
# Podman Run with Security Hardening
# ============================================================================
# Note: Podman runs rootless by default, providing additional isolation
# compared to Docker without requiring privileged daemon

podman run --rm -it \
    `# ---- Security Hardening ----` \
    --security-opt=no-new-privileges:true \
    --cap-drop=ALL \
    --read-only \
    \
    `# ---- User Namespace ----` \
    --user "${USER_UID}:${USER_GID}" \
    ${VIDEO_GROUPS} \
    ${AUDIO_GROUPS} \
    \
    `# ---- Writable Mounts ----` \
    --tmpfs /tmp:rw,noexec,nosuid,size=1g \
    --mount type=tmpfs,destination=/home/nonroot/.config,tmpfs-mode=0700 \
    -v "${HOME}/.stremio-server:/home/nonroot/.stremio-server:rw" \
    \
    `# ---- X11 Display ----` \
    -e DISPLAY="${DISPLAY}" \
    -v /tmp/.X11-unix:/tmp/.X11-unix:ro \
    -v "${XAUTH_TMP}:/tmp/.docker.xauth:ro" \
    -e XAUTHORITY=/tmp/.docker.xauth \
    \
    `# ---- PulseAudio ----` \
    -v "/run/user/${USER_UID}/pulse:/run/user/${USER_UID}/pulse:ro" \
    -e PULSE_SERVER="unix:/run/user/${USER_UID}/pulse/native" \
    \
    `# ---- Device Access ----` \
    --device /dev/dri:/dev/dri:rw \
    --device /dev/snd:/dev/snd:rw \
    \
    `# ---- Network ----` \
    --network=host \
    \
    `# ---- Environment ----` \
    -e QT_X11_NO_MITSHM=1 \
    -e QT_QUICK_BACKEND=software \
    \
    `# ---- Resource Limits ----` \
    --memory="2g" \
    --memory-swap="2g" \
    --cpus="2.0" \
    --pids-limit=200 \
    \
    `# ---- Container Name ----` \
    --name stremio-podman-$$ \
    \
    "${IMAGE_NAME}" "$@"

echo "✅ Stremio exited successfully"
